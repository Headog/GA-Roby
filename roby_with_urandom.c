...
int urandom_f() {
  static int buffer;
  fread(&buffer, sizeof(buffer), 1, urandom);
  if (buffer < 0) {
    return -buffer;
  }
  return buffer;
...
void create_random_world(bool world[HEIGHT][WIDTH]) {
  ...
  for (i = 0; i < CANS; i++) {
    do {
      target = world[urandom_f() % HEIGHT] + urandom_f() % WIDTH;
    } while (*target);
    *target = true;
  }
}
...
int act(int *x, int *y, bool world[HEIGHT][WIDTH], const enum action action) {
  ...
  case move_random:
    return act(x, y, world, urandom_f() % _DIRECTIONS);
  }
  return 0;
}
...
void mix_rules(const enum action rule1[_SITUATIONS],
  ...
  int i;
  int split = urandom_f() % (_SITUATIONS - 1);
  ...
  memcpy(result2 + split, rule1 + split,
         (_SITUATIONS - split) * sizeof(enum action));
  for (i = 0; i < MIXES; i++) {
    result1[urandom_f() % _SITUATIONS] = urandom_f() % _ACTIONS;
    result2[urandom_f() % _SITUATIONS] = urandom_f() % _ACTIONS;
  }
}
...
void train(struct history *history, FILE *log_file) {
  ...
  for (i = 0; i < EPOCHS; i++) {
    ...
    for (j = 0; j < RULES - 1; j += 2) {
      mix_rules(results[WEIGHTED(urandom_f() % RULES, RULES)].rule,
                results[WEIGHTED(urandom_f() % RULES, RULES)].rule, rules[j],
                rules[j + 1]);
    }
  }
}
